...
class ResponseTemplate
{
    ...
    {
        $class = $name;
        $suffix = config('plugin.blankkids.webman-build.app.file_name_format.response', '');
        $file_path = config('plugin.blankkids.webman-build.app.child_path.response', '');
    ...
    {
        list($baseClass, $basenamespace, $basefile) = self::createBase($name, $module_name);
...
EOF;
    ...
    {
        $config = self::getConfig('Api', 'common');
        $class = $config['class'];
        $namespace = $config['namespace'];
        $file = $config['file'];
